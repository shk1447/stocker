        ...
        public static string GetErrorMessage(int nErrorCode)
        {
            switch (nErrorCode)
            {
                case KOAErrorCode.OP_ERR_NONE:
                    return "[" + nErrorCode.ToString() + "] :" + "정상처리";                    
                case KOAErrorCode.OP_ERR_LOGIN:
                    return "[" + nErrorCode.ToString() + "] :" + "사용자정보교환에 실패하였습니다. 잠시 후 다시 시작하여 주십시오.";
                case KOAErrorCode.OP_ERR_CONNECT:
                    return "[" + nErrorCode.ToString() + "] :" + "서버 접속 실패";
                case KOAErrorCode.OP_ERR_VERSION:
                    return "[" + nErrorCode.ToString() + "] :" + "버전처리가 실패하였습니다";
                case KOAErrorCode.OP_ERR_SISE_OVERFLOW:
                    return "[" + nErrorCode.ToString() + "] :" + "시세조회 과부하";
                case KOAErrorCode.OP_ERR_RQ_STRUCT_FAIL:
                    return "[" + nErrorCode.ToString() + "] :" + "REQUEST_INPUT_st Failed";
                case KOAErrorCode.OP_ERR_RQ_STRING_FAIL:
                    return "[" + nErrorCode.ToString() + "] :" + "요청 전문 작성 실패";
                case KOAErrorCode.OP_ERR_ORD_WRONG_INPUT:
                    return "[" + nErrorCode.ToString() + "] :" + "주문 입력값 오류";
                case KOAErrorCode.OP_ERR_ORD_WRONG_ACCNO:
                    return "[" + nErrorCode.ToString() + "] :" + "계좌비밀번호를 입력하십시오.";
                case KOAErrorCode.OP_ERR_OTHER_ACC_USE:
                    return "[" + nErrorCode.ToString() + "] :" + "타인계좌는 사용할 수 없습니다.";
                case KOAErrorCode.OP_ERR_MIS_2BILL_EXC:
                    return "[" + nErrorCode.ToString() + "] :" + "주문가격이 20억원을 초과합니다.";
                case KOAErrorCode.OP_ERR_MIS_5BILL_EXC:
                    return "[" + nErrorCode.ToString() + "] :" + "주문가격은 50억원을 초과할 수 없습니다.";
                case KOAErrorCode.OP_ERR_MIS_1PER_EXC:
                    return "[" + nErrorCode.ToString() + "] :" + "주문수량이 총발행주수의 1%를 초과합니다.";
                case KOAErrorCode.OP_ERR_MID_3PER_EXC:
                    return "[" + nErrorCode.ToString() + "] :" + "주문수량은 총발행주수의 3%를 초과할 수 없습니다";
                default:
                    return "[" + nErrorCode.ToString() + "] :" + "알려지지 않은 오류입니다.";
            }
        }
    }
}
