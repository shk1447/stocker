...
module.exports = function() {
        ...
        return new Promise((resolve, reject) => {
            async function page_req(num,code,rows) {
                try {
                    ...
                    var row = {};
                    var header = ["Date", "Close", "Rate", "Open", "High", "Low", "Volume"]
    ...
    function publish() {
        try {
            _.each(code_list, (v,k) => {
                if(v.length > 0) {
                    getSise(k, 1).then((data) => {
                        _.each(v, (socket, i) => {
                            console.log(k);
                            data["id"] = k;
    ...
    function subscribe(socket, options) {
        _.each(options.code_list, (code,i) => {
            if(code_list[code]) {
                code_list[code].push(socket);
            } else {
                code_list[code] = [ socket ];
            }
    ...
    function unsubscribe(socket) {
        var idx = subscriber.indexOf(socket);
        if( idx > -1 ) subscriber.splice(idx);
        _.each(code_list, (v,i) => {
            var rm_idx = v.indexOf(socket);
            if(rm_idx > -1)  v.splice(rm_idx);
        })
    }
    return {
        subscribe:subscribe,
